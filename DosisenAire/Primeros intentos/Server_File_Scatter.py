...
MixedFFF = Mixed[Mixed['FFF']==1].reset_index(drop=True)
...
for i in np.arange(number, Mixed.shape[0]):
    if Mixed[['FFF']].loc[i].to_numpy() == 0:
        BigZ.append((0.25*50.9*42.6)/(Mixed['Z'].loc[i]**2))
    else:
        BigZ.append((0.25*(50.9-z_s)*(42.6-z_s))/(Mixed['Z'].loc[i]**2-z_s))
